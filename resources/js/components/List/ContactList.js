...
function ContactList(props) {
    const [contacts, setLeads] = useState([]);
    ...
    useEffect(() => {
        ...
        props.setActiveComponentProp('ContactList');
    ...
    const loadData = () => {
        setIsLoading(true);
        axios.get('/api/v1/contato/lista?page='+state.currentPage, {
            params: {
            ...
            buttons: [
                ...
                label: 'Sim',
                    onClick: () => {
                        ...
                        axios.post('/api/v1/contato/apagar', {
                            api_token: authUser.api_token,
                            lead_id: id
                        })
                        .then(response => {
    ...
    const dataTable = () => {
        ...
        (contacts.length == 0 ? <div className="text-center text-gray">
                                <div className="p-3 font-weight-bold">Nenhum dado disponível</div>
                            </div> :
        contacts.map((lead, i) => {
            return <ContactItem onClickDeleteHandler={onClickDeleteHandler} obj={lead} key={i} />;
        }));
    }
            ...
            <div className="card animated fadeIn">
                <div className="card-body">
                    <TopControl
                        ...
                        query={state.query}
                    />
                    <div className='container-list-wrapper bg-gradient-light'>
                            {dataTable()}
                    </div>
