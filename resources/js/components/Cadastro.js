...
class Cadastro extends Component {
    ...
    onSubmitHandle = (e) =>{
        ...
        if (this.validator.allValid()) {
            ...
            axios.post('/cadastro', $(e.target).serialize())
            .then(response => {
                this.setState({
                    loading: false
                });
                    ...
                    <div className="auth-form-light text-left p-5 animated fadeIn rounded">
                        ...
                        <form className="pt-3" ref={c => { this.form = c }} onSubmit={this.onSubmitHandle}>
                            <div className="form-group">
                                <input type="text" className="form-control form-control-lg" name="name" id="name" placeholder="Nome Completo" value={this.state.name} onChange={this.onChangeHandle}/>
                            ...
                            <div className="form-group">
                                <input type="password" className="form-control form-control-lg" name="password_confirmation" id="password_confirmation" placeholder="Confirme a Senha" value={this.state.password_confirmation} onChange={this.onChangeHandle}/>
                                {this.validator.message('confirm password', this.state.password_confirmation, 'required|confirmPassword:'+this.state.password)}
                            </div>
                            <div className="mt-3">
                                <button type="submit" className="btn btn-block btn-gradient-primary btn-lg font-weight-medium auth-form-btn" >Cadastrar</button>
                            </div>
                            <div className="text-center mt-4 font-weight-light"> Já tem cadastro? <Link to='/login' className="text-primary">Login</Link>
                            </div>
                        </form>
                    </div>
                </LoadingOverlay>
